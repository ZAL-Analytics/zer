...
use crate::{
    config::{BatchStartupMode, LinkMode},
    pipeline::Pipeline,
...
macro_rules! emit {
    ($pipeline:expr, $event:expr) => {
        if let Some(tx) = &$pipeline.progress {
            let _ = tx.send($event);
        }
    };
}
...
impl Pipeline {
    ...
    pub async fn run_batch(&self, records: Vec<Record>) -> Result<BatchReport, ZerError> {
        ...
        if self.config.link_mode == LinkMode::LinkOnly {
            pair_indices
                .retain(|&(a, b)| records[a].source.as_deref() != records[b].source.as_deref());
        }
        ...
        let batch = match &self.mapped_comparator {
            ...
            None => {
                ...
                self.comparator
                    .compare_batch_from_pool(&pool, &pair_indices, &self.schema)
            }
        };
        ...
        let (params, em_iterations) = match startup_mode {
            ...
            RegistryStartupMode::WarmStart { artifact, distance } => {
                ...
                if batch.n_pairs == 0 {
                    ...
                    emit!(
                        self,
                        PipelineEvent::EmComplete {
                            iterations: self.config.em_max_iter_warm
                        }
                    );
            ...
            RegistryStartupMode::ColdStart => {
                ...
                if batch.n_pairs == 0 {
                    ...
                    let p =
                        self.scorer
                            .estimate_params(&batch, None, self.config.em_max_iter_cold)?;
                    emit!(
                        self,
                        PipelineEvent::EmComplete {
                            iterations: self.config.em_max_iter_cold
                        }
                    );
        ...
        let scored_pairs: Vec<(RecordId, RecordId, f32)> = if cfg!(feature = "collect-pairs") {
            scored
                .iter()
                .map(|sp| {
...
fn apply_judge(
    scored: &mut [zer_core::scoring::ScoredPair],
    judge: &dyn Judge,
) -> Result<(usize, usize), ZerError> {
    use zer_core::traits::JudgeVerdict;

    let borderline_indices: Vec<usize> = scored
        .iter()
        .enumerate()
    ...
    let borderlines: Vec<_> = borderline_indices
        .iter()
    ...
    for (&idx, verdict) in borderline_indices.iter().zip(verdicts) {
        match verdict {
            JudgeVerdict::IncreaseConfidence => {
                scored[idx].band = MatchBand::AutoMatch;
                promoted += 1;
            }
            JudgeVerdict::DecreaseConfidence => {
                scored[idx].band = MatchBand::AutoReject;
                demoted += 1;
            }
            _ => {}
        }
...
mod tests {
    ...
    async fn duplicate_records_produce_candidates() {
        ...
        assert!(
            ...
            "identical records should block together"
        );
    }
    ...
    fn make_pipeline_with_mode(
        dir: &TempDir,
        link_mode: crate::config::LinkMode,
    ) -> std::sync::Arc<Pipeline> {
        Pipeline::builder()
            .schema(person_schema())
            .store(ZalEntityStore::open_in_memory().unwrap())
            .config(PipelineConfig {
    ...
    async fn link_only_filters_within_source_pairs() {
        ...
        assert_eq!(
            ...
            "LinkOnly must not produce within-source pairs"
        );
    }
    ...
    async fn link_only_allows_cross_source_pairs() {
        ...
        for i in 1..=3 {
            records.push(make_record_with_source(
                i,
                "Jan",
                "de Vries",
                "1985-03-15",
        ...
        assert!(
            ...
            "LinkOnly must produce cross-source pairs when sources differ"
        );
        assert_eq!(
            report.within_source_pairs, 0,
            "LinkOnly must not produce within-source pairs"
