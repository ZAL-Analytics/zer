...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(report.within_source_pairs > 0,
            "LinkAndDedupe must include within-source pairs (BRP has a duplicate)");
    assert!(report.cross_source_pairs > 0,
    ...
    for (entity, records) in &view {
        let sources: Vec<_> = records.iter()
            .filter_map(|r| r.source.as_deref())
            .collect();
        println!("  entity={} | {} member(s) | sources: {:?}", entity.id, records.len(), sources);
    }
