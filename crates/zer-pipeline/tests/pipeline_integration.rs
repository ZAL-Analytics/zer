...
use zer_pipeline::{
    config::{BatchStartupMode, PipelineConfig},
    ingester::Ingester,
    pipeline::Pipeline,
    BatchReport,
};
...
async fn batch_single_record_creates_one_entity() {
    ...
    let pipeline = make_pipeline(&dir);
    let report = pipeline
        .run_batch(vec![make_record(1, "Alice", "Smith", "1990-01-01")])
        .await
        .unwrap();
...
async fn batch_identical_records_have_candidates() {
    ...
    assert!(
        ...
        "identical records must produce candidates"
    );
}
...
async fn batch_diverse_records_produce_report() {
    ...
    assert_eq!(
        report.candidate_pairs, 0,
        "unrelated records should not block together"
    );
}
...
async fn ingester_stream_returns_correct_ids() {
    ...
    for i in 1u64..=8 {
        ...
        assert_eq!(
            result.record_id, i,
            "result must correspond to the sent record"
        );
    }
}
...
async fn ingester_flush_completes_without_error() {
    ...
    let ingester = Arc::clone(&pipeline).ingester();
    ingester
        .send(make_record(1, "Anna", "Berg", "1991-04-11"))
        .await
        .unwrap();
