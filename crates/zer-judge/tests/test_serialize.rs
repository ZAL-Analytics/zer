...
fn output_has_exactly_two_sep_tokens() {
    ...
    let s = serialize_pair(&r, &r, &schema);
    assert_eq!(s.matches("[SEP]").count(), 2, "expected exactly two [SEP]: {s}");
}
...
fn both_records_appear_in_output() {
    ...
    assert!(s.contains("COL:naam VAL:jan smits"), "left record missing: {s}");
    assert!(s.contains("COL:naam VAL:jan smyts"), "right record missing: {s}");
}
...
fn fields_appear_in_schema_order_regardless_of_insertion_order() {
    ...
    let dob_pos  = s.find("COL:geboortedatum").expect("COL:geboortedatum missing");
    assert!(naam_pos < dob_pos, "naam must appear before geboortedatum (schema order): {s}");
}
