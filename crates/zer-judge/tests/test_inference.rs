...
fn minilm_empty_pairs_returns_empty_verdicts() {
    if !std::path::Path::new(MINILM_DIR).exists() {
        return;
    }
...
fn minilm_identical_records_produce_increase_confidence() {
    ...
    assert!(
        ...
        "identical records should produce IncreaseConfidence, got {:?}",
        verdicts[0],
    );
}
...
fn minilm_batch_returns_one_verdict_per_pair() {
    ...
    let records: Vec<Record> = (1..=6)
        .map(|i| Record::new(i).insert("naam", FieldValue::Text(format!("person {i}"))))
        .collect();
...
fn minilm_clone_shares_worker_thread() {
    ...
    let v1 = judge
        .adjudicate(&[scored_pair(1, 2)])
        .expect("adjudicate on original failed");
    let v2 = judge2
        .adjudicate(&[scored_pair(1, 2)])
        .expect("adjudicate on clone failed");
    assert_eq!(v1.len(), 1);
    assert_eq!(v2.len(), 1);
}
...
fn minilm_adjudicate_from_tokio_via_spawn_blocking() {
    ...
    let verdicts = rt.block_on(async {
        ...
        tokio::task::spawn_blocking(move || judge.adjudicate(&[scored_pair(1, 2)]))
            .await
...
fn deberta_base_inference_runs_on_cpu() {
    if !std::path::Path::new(DEBERTA_BASE_DIR).exists() {
        return;
    }
