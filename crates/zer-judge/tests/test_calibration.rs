...
fn default_lr_increase_is_above_one() {
    ...
    assert!(
        table.lr_increase > 1.0,
        "lr_increase must boost probability; got {}",
        table.lr_increase
    );
}
...
fn default_lr_decrease_is_below_one() {
    let table = CalibrationTable::default();
    assert!(
        table.lr_decrease < 1.0,
        "lr_decrease must reduce probability; got {}",
        table.lr_decrease
    );
}
...
fn clone_is_independent() {
    ...
    assert_ne!(
        original.lr_increase, cloned.lr_increase,
        "clone must be a deep copy"
    );
}
