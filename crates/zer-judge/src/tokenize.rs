...
fn load_from_file(path: &Path) -> Result<Tokenizer, crate::error::JudgeError> {
    Tokenizer::from_file(path).map_err(|e| crate::error::JudgeError::Tokenizer(e.to_string()))
}
...
mod tests {
    ...
    fn encode_batch_shape_matches_max_length() {
        let tok = match try_minilm_tokenizer() {
            Some(t) => t,
            None => return,
        };
    ...
    fn encode_batch_attention_mask_nonzero_for_tokens() {
        ...
        assert!(
            nonzero_count > 0,
            "attention mask should have non-zero entries"
        );
    }
    ...
    fn encode_batch_padding_positions_are_zero() {
        ...
        let texts = vec![
            "hi".to_string(),
            "a b c d e f g h i j k l m n o p q r s t".to_string(),
        ];
    ...
    fn huggingface_source_returns_error() {
        ...
        let spec = HubSpec {
            source: TokenizerSource::HuggingFace("test/model".into()),
        };
