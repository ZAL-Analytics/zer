...
fn brp_em_converges_and_params_are_plausible() {
    ...
    assert_eq!(
        vectors.n_pairs,
        all_pairs.len(),
        "compare_batch should produce one vector per pair"
    );
    assert_eq!(
        vectors.n_fields,
        schema.len(),
        "each vector should have one level per field"
    );
...
fn brp_precision_recall_at_threshold() {
    ...
    let true_match_predicted_match = scored[..match_count]
        .iter()
        .filter(|s| s.band == MatchBand::AutoMatch)
        .count();
    let true_nonmatch_predicted_match = scored[match_count..]
        .iter()
        .filter(|s| s.band == MatchBand::AutoMatch)
        .count();
...
fn brp_comparison_vector_fields_populated() {
    ...
    assert!(
        non_none > 0,
        "True pair should have at least one non-None comparison level"
    );
}
