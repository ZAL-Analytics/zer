...
fn hks_gt_csv() -> std::path::PathBuf {
    zer_test_utils::dataset_path(
        ...
        "tests/hks/ground_truth_pairs.csv",
    )
}
...
fn load_hks_records() -> HashMap<String, Record> {
    let mut rdr =
        csv::Reader::from_path(hks_csv()).expect("HKS CSV not found, run data generator first");
...
fn load_true_pairs(records: &HashMap<String, Record>) -> Vec<(RecordId, RecordId)> {
    let mut rdr = csv::Reader::from_path(hks_gt_csv()).expect("HKS ground truth CSV not found");
    let mut pairs = vec![];
...
fn hks_alias_field_contributes_non_none_for_alias_pairs() {
    ...
    let alias_idx = schema
        .fields
        .iter()
        .position(|f| f.name == "alias_namen")
        .unwrap();
...
fn hks_em_converges_on_alias_data() {
    ...
    assert!(
        ...
        schema.len(),
        (0..schema.len())
            .map(|f| (params.m[f][exact_idx], params.u[f][exact_idx]))
            .collect::<Vec<_>>()
    );
}
