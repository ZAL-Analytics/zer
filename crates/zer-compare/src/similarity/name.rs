    ...
    fn similarity(&self, a: &FieldValue, b: &FieldValue) -> f32 {
        match (a, b) {
            (FieldValue::Text(a), FieldValue::Text(b)) => {
                ...
                if norm_a.is_empty() || norm_b.is_empty() { return 0.0; }
...
impl SimilarityFn for TokenOverlapSimilarity {
    ...
    fn similarity_str(&self, a: &str, b: &str) -> f32 { jaccard_tokens(a, b) }
    fn field_kind(&self) -> FieldKind { FieldKind::Name }
}
...
impl SimilarityFn for LevenshteinSimilarity {
    ...
    fn similarity_str(&self, a: &str, b: &str) -> f32 {
        ...
        if dist > self.max_distance { 0.0 }
        else { 1.0 - (dist as f32 / self.max_distance.max(1) as f32) }
    }
    fn field_kind(&self) -> zer_core::schema::FieldKind { zer_core::schema::FieldKind::Name }
}
...
impl SimilarityFn for AliasTokenOverlapSimilarity {
    fn similarity(&self, a: &FieldValue, b: &FieldValue) -> f32 {
        match (a, b) {
            (FieldValue::Text(a), FieldValue::Text(b)) => {
                let aliases_a: Vec<&str> = a.split('|').map(str::trim).filter(|s| !s.is_empty()).collect();
                let aliases_b: Vec<&str> = b.split('|').map(str::trim).filter(|s| !s.is_empty()).collect();
                if aliases_a.is_empty() || aliases_b.is_empty() { return 0.0; }
                aliases_a.iter()
    ...
    fn field_kind(&self) -> FieldKind { FieldKind::Alias }
}
...
mod tests {
    ...
    fn phonetic_equality_sound_alikes() {
        ...
        assert_eq!(sim.similarity(&a, &b), 1.0, "Smith and Smyth should be phonetically equal");
    }
    ...
    fn phonetic_equality_different() {
        ...
        assert_eq!(sim.similarity(&a, &b), 0.0, "Jansen and Bakker should not be phonetically equal");
    }
    ...
    fn token_overlap_swapped_name() {
        ...
        assert_eq!(sim.similarity(&a, &b), 1.0, "token overlap should be 1.0 for swapped tokens");
    }
    ...
    fn token_overlap_partial() {
        ...
        let s   = sim.similarity(&a, &b);
        assert!(s > 0.3, "partial name overlap should produce > 0.3, got {s}");
    }
    ...
    fn similarity_null_fields_return_zero() {
        let sim = JaroWinklerSimilarity;
        assert_eq!(sim.similarity(&FieldValue::Null, &FieldValue::Text("test".into())), 0.0);
        assert_eq!(sim.similarity(&FieldValue::Text("test".into()), &FieldValue::Null), 0.0);
    }
    ...
    fn levenshtein_exact_match() {
        ...
        let a   = FieldValue::Text("Jansen".into());
        let b   = FieldValue::Text("Jansen".into());
        assert_eq!(sim.similarity(&a, &b), 1.0, "edit distance 0 must yield 1.0");
    }
    ...
    fn levenshtein_over_max() {
        ...
        assert_eq!(sim.similarity(&a, &b), 0.0, "edit distance > max_distance must yield 0.0");
    }
    ...
    fn levenshtein_partial() {
        ...
        let s   = sim.similarity(&a, &b);
        assert!(s > 0.0 && s < 1.0, "partial distance must yield value in (0.0, 1.0), got {s}");
    }
