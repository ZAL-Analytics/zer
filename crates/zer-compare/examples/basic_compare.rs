...
fn main() {
    ...
    let indices: Vec<(usize, usize)> = (0..training_pairs.len())
        .map(|i| (i * 2, i * 2 + 1))
        .collect();
    ...
    println!(
        "  log_prior_odds={:.3}  upper={:.3}  lower={:.3}",
    ...
    println!("\nScored pairs:");
    println!(
        "  Alice canonical vs Alice variant: probability={:.4}  band={:?}",
        scored_match.match_probability, scored_match.band
    );
    println!(
        "  Alice canonical vs Bob:           probability={:.4}  band={:?}",
