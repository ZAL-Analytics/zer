...
use zer_blocking::{
    ...
    keys::{
        AliasPhoneticKey, DateFragmentKey, DateGranularity, FuzzyYearKey,
        LicensePlateNormKey, PhoneticNameDobKey, PlateOCRFuzzyKey, SuffixKey,
    },
};
use zer_core::{
    record::Record,
...
fn person_blocking_demo() {
    ...
    let manual = CompositeBlocker::new()
        ...
        .add(DateFragmentKey::new("geboortedatum", DateGranularity::YearMonth));
    ...
    assert!(cands.contains(&2), "r2 (same person) must be a candidate of r1");
    assert!(!cands.contains(&3), "r3 (unrelated) must NOT be a candidate of r1");
...
fn custom_category_demo() {
    ...
    assert!(cands2.contains(&2), "r2 (same postcode) must be a candidate under postcode-only");
    assert!(!cands2.contains(&3), "r3 (different postcode) must NOT be a candidate");
    ...
    let escape_hatch = CustomSchemaCategory::new()
    ...
    assert!(!keys3.is_empty(), "escape-hatch key must produce at least one key");
    println!("  Escape-hatch with_key: {} key(s) produced. ✓", keys3.len());
}
