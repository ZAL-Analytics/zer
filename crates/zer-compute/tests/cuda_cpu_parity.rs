...
fn assert_parity(
    ...
    assert_eq!(cuda_batch.n_pairs, ref_batch.n_pairs, "output length mismatch");
...
fn pair2(id_a: u64, id_b: u64, va0: &str, vb0: &str, va1: &str, vb1: &str)
    -> (Record, Record)
{
    let a = Record::new(id_a).insert("f0", tv(va0)).insert("f1", tv(va1));
    let b = Record::new(id_b).insert("f0", tv(vb0)).insert("f1", tv(vb1));
    (a, b)
}
...
fn parity_large_batch_crosses_warp_boundaries() {
    ...
    let schema = schema1(FieldKind::Name);
    let names_a = ["Jan", "Piet", "Klaas", "Marie", "Anna", "Dirk", "Henk", "Lena"];
    let names_b = ["Jan", "Piet", "Klaas", "Maria", "Anne", "Dick", "Henk", "Lena"];
...
fn cpu_device_comparator_matches_field_comparator_all_kinds() {
    ...
    let ref_batch    = FieldComparator::from_schema(&schema).compare_batch_from_pool(&pool, &indices, &schema);
