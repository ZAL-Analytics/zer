...
fn gpu_comparator_cpu_matches_field_comparator() {
    ...
    let sample: Vec<(Record, Record)> = true_pairs.iter().take(50)
...
fn gpu_scorer_cpu_matches_fellegi_sunter() {
    ...
    let all_pairs: Vec<(Record, Record)> = true_pairs.iter().take(200)
    ...
    let params     = ref_scorer.estimate_params(&vectors, None, 50)
    ...
    for (g, r) in gpu_scores.iter().zip(ref_scores.iter()) {
        assert!(
            ...
            g.match_probability, r.match_probability
        );
    }
}
...
fn gpu_scorer_em_converges_on_brp_data() {
    ...
    let all_pairs: Vec<(Record, Record)> = true_pairs.iter()
    ...
    let params = gpu_scorer.estimate_params(&vectors, None, 100)
