...
fn anpr_gt_csv() -> std::path::PathBuf {
    zer_test_utils::dataset_path(
        env!("CARGO_MANIFEST_DIR"),
        "tests/anpr/ground_truth_compare_pairs.csv",
    )
}
...
fn pairs_to_records(
    ...
    id_pairs
        .iter()
        .filter_map(|(a, b)| Some(((*id_map.get(a)?).clone(), (*id_map.get(b)?).clone())))
        .collect()
}
...
fn load_anpr_pairs(_schema: &Schema) -> Vec<(Record, Record)> {
    ...
    for row in rdr.records().flatten() {
        ...
        if true_plate.is_empty() || ocr_plate.is_empty() {
            continue;
        }

        let a = Record::new(id).insert("kenteken", FieldValue::Text(true_plate.into()));
        let b = Record::new(id + 1).insert("kenteken", FieldValue::Text(ocr_plate.into()));
        out.push((a, b));
        id += 2;
    }
    out
...
fn batch_chunking_is_result_stable() {
    ...
    let sample: Vec<(Record, Record)> =
        pairs_to_records(&true_ids[..true_ids.len().min(200)], &id_map);
...
fn anpr_ocr_pairs_score_higher_than_random() {
    ...
    let ocr_close_or_exact = (0..ocr_vecs.n_pairs)
        .filter(|&p| {
            matches!(
                ocr_vecs.level(0, p),
                ComparisonLevel::Exact | ComparisonLevel::Close
            )
        })
        .count();
    ...
    let indices_r: Vec<(usize, usize)> = (0..random_pairs.len())
        .map(|i| (i * 2, i * 2 + 1))
        .collect();
    ...
    println!(
        ...
        ocr_close_or_exact, ocr_vecs.n_pairs, random_close_or_exact, random_vecs.n_pairs
    );
