...
impl Comparator for DeviceComparator {
    ...
    fn compare_batch_from_pool(
        ...
        self.cpu_fallback.compare_batch_from_pool(pool, indices, schema)
    }
}
...
mod tests {
    ...
    fn small_batch_uses_cpu_fallback() {
        ...
        let records: Vec<Record> = (0..20).map(|i| {
            if i % 2 == 0 { make_record(i) } else { make_record_b(i) }
        }).collect();
    ...
    fn exact_match_produces_exact_levels() {
        ...
        for level in &vec.levels {
            assert_eq!(*level, ComparisonLevel::Exact, "identical records should give Exact");
        }
    }
    ...
    fn synthetic_records(n: usize, schema: &Schema) -> Vec<Record> {
        ...
        let names  = ["Alice", "Alicia", "Bob", "Robert", "Eva", "Eva-Marie",
                      "Jan", "Johan", "Petra", "Pietra", "Lena", "Lena-Marie"];
        let dates  = ["1990-01-15", "1990-01-16", "1985-06-20", "1975-03-03",
                      "2000-12-31", "2001-01-01", "1960-07-07", "1970-11-22"];
        let plates = ["12-ABC-3", "12-ABD-3", "45-XYZ-6", "46-XYZ-6",
                      "AB-123-C", "AB-124-C", "ZZ-999-Z", "ZZ-998-Z"];
    ...
    fn large_batch_auto_detect_returns_correct_count() {
        ...
        assert_eq!(batch.n_pairs,  2_000, "compare_batch_from_pool must return one row per pair");
        assert_eq!(batch.n_fields, 3,     "each batch must have one column per field");
    }
