...
impl KernelDispatch<EmReduce> for VulkanDevice {
    fn dispatch(&self, input: EmReduceInput<'_>) -> Result<EmReduceOutput, GpuError> {
        ...
        unsafe {
            let _ = self
                .device
                .free_descriptor_sets(self.descriptor_pool, &[ds_partial, ds_final]);
        }
        {
            let mut a = self.allocator.lock().unwrap();
    ...
    pub(crate) fn destroy(self, device: &ash::Device, allocator: &mut Allocator) {
        unsafe {
            let _ = device.free_descriptor_sets(
                ...
                &[self.ds_estep, self.ds_partial, self.ds_final],
            );
...
impl VulkanDevice {
    ...
    pub(crate) fn em_init_session(
        ...
        let (
            ...
            (
                ...
                staging!(
                    (n_fields * NUM_LEVELS) as u64 * 4,
                    "em_sess_staging_weights"
                ),
        ...
        let estep_writes: Vec<vk::WriteDescriptorSet> = ei
            .iter()
            .enumerate()
            .map(|(i, info)| {
                vk::WriteDescriptorSet::default()
    ...
    pub(crate) fn em_run_iteration(
        ...
        unsafe {
            ...
            for buf in [
                &session.d_m_partials,
                &session.d_u_partials,
                &session.d_match_totals,
                &session.d_nonmatch_totals,
            ] {
                buffer_barrier(
                    device,
                    cmd,
            ...
            for buf in [
                &session.d_m_out,
                &session.d_u_out,
                &session.d_total_match,
                &session.d_total_nonmatch,
            ] {
                buffer_barrier(
                    device,
                    cmd,
            ...
            cmd_copy_buffer(
                ...
                cmd,
                &session.d_total_match,
                &session.staging_total_match,
            );
            cmd_copy_buffer(
                device,
                cmd,
                &session.d_total_nonmatch,
                &session.staging_total_nonmatch,
            );
        ...
        unsafe {
            device
                .reset_fences(&[session.fence])
                .map_err(|e| vk_err("reset_fences", e))?;
            let submit =
                vk::SubmitInfo::default().command_buffers(std::slice::from_ref(&session.cmd));
            device
                .queue_submit(
                    self.compute_queue,
            ...
            device
                .wait_for_fences(&[session.fence], true, u64::MAX)
