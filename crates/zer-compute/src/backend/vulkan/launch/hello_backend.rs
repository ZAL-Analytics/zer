    ...
    fn dispatch(&self, _input: HelloBackendInput) -> Result<HelloBackendOutput, GpuError> {
        ...
        let (d_out, staging_out) = {
            ...
            let d = VulkanBuffer::new_device_local(
                &self.device, &mut alloc, 4,
                vk::BufferUsageFlags::STORAGE_BUFFER, "hello_d_out",
            )?;
        ...
        let out_buf_info = [vk::DescriptorBufferInfo::default()
            .buffer(d_out.buffer).offset(0).range(vk::WHOLE_SIZE)];
        ...
        one_shot_submit(device, self.command_pool, self.compute_queue, |cmd| {
            unsafe {
                ...
                device.cmd_bind_descriptor_sets(
                    cmd, vk::PipelineBindPoint::COMPUTE, layout, 0, &[ds], &[],
                );
                let dummy: u32 = 0;
                device.cmd_push_constants(
                    cmd, layout, vk::ShaderStageFlags::COMPUTE, 0, &dummy.to_ne_bytes(),
                );
        ...
        unsafe { let _ = self.device.free_descriptor_sets(self.descriptor_pool, &[ds]); }
        {
            let mut alloc = self.allocator.lock().unwrap();
