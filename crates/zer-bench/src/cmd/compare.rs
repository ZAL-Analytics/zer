...
pub fn print_comparison_for_dir(dir: &str, since: std::time::SystemTime) -> anyhow::Result<()> {
    ...
    if rows.is_empty() {
        eprintln!(
            "warning: no summary files found for inline comparison  dir={}",
            results_dir.display()
        );
        return Ok(());
    }
...
pub fn run(args: CompareArgs) -> anyhow::Result<()> {
    ...
    if rows.is_empty() {
        eprintln!(
            "warning: no matching summary files found  dir={}  mode={}  dataset={}",
            results_dir.display(),
            args.mode,
            args.dataset
        );
    ...
    let out_dir = resolve_out_dir(args.out.as_deref().unwrap_or(&args.results));
...
fn load_summary_rows(
    ...
    for entry in entries.flatten() {
        ...
        for result in rdr.deserialize() {
            let row: SummaryRow =
                result.map_err(|e| anyhow::anyhow!("parse error in {}: {e}", path.display()))?;
    ...
    rows.sort_by(|a, b| (&a.mode, &a.dataset, &a.library).cmp(&(&b.mode, &b.dataset, &b.library)));
...
fn write_combined_csv(
    ...
    let mode_slug = if mode.is_empty() {
        "all".into()
    } else {
        mode.replace('-', "_")
    };
    let dataset_slug = if dataset.is_empty() {
        "all".into()
    } else {
        dataset.replace('-', "_")
    };
