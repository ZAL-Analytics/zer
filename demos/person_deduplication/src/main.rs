...
use demo_common::{
    ...
    print_cluster_tree, print_score_histogram,
    viz::{ClusterEntry},
};
...
fn load_ground_truth(path: &Path) -> HashSet<(u64, u64)> {
    ...
    rdr.deserialize::<GroundTruthRow>()
        .map(|r| {
            ...
            (row.record_id_a.min(row.record_id_b), row.record_id_a.max(row.record_id_b))
        })
        .collect()
}
...
async fn main() {
    ...
    let multi_member: Vec<_> = clusters.iter().filter(|(e, _)| e.members.len() > 1).collect();
    ...
    let entries: Vec<ClusterEntry> = multi_member.iter().take(20).map(|(entity, recs)| {
        ...
        let labels: Vec<String> = recs.iter().map(|r| {
            let naam = format!(
                "{} {}",
        ...
        ClusterEntry { entity_id: entity.id, record_ids, best_scores, labels }
    }).collect();
