...
use demo_common::{
    ...
    print_comparison_vectors, print_score_histogram,
    viz::FieldComparison,
};
...
fn show_pair(
    ...
    let field_comparisons: Vec<FieldComparison> = schema.fields.iter()
...
fn main() {
    ...
    println!("Thresholds: match ≥ {:.2}, reject < {:.2}", params.upper_threshold, params.lower_threshold);
    ...
    all_scores.push(show_pair("Exact duplicate", &a1, &b1, &comparator, &scorer, &params, &schema));
    ...
    all_scores.push(show_pair("Nickname + address move", &a2, &b2, &comparator, &scorer, &params, &schema));
    ...
    all_scores.push(show_pair("Maiden name change", &a3, &b3, &comparator, &scorer, &params, &schema));
    ...
    all_scores.push(show_pair("DOB transposition", &a4, &b4, &comparator, &scorer, &params, &schema));
    ...
    all_scores.push(show_pair("Different persons", &a5, &b5, &comparator, &scorer, &params, &schema));
    ...
    all_scores.push(show_pair("Phonetic name variant", &a6, &b6, &comparator, &scorer, &params, &schema));
